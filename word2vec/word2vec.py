...
def negSamplingLossAndGradient(
    ...
    neg_u_k = outsideVectors[negSampleWordIndices]
    z_o = sigmoid(np.dot(u_o, centerWordVec))
    z_k = sigmoid(-np.dot(neg_u_k, centerWordVec))
    
    loss = -np.log(z_o) - np.sum(np.log(z_k))

    gradCenterVec = (z_o - 1) * u_o + np.dot((1 - z_k), neg_u_k)
    
    gradOutsideVecs = np.zeros_like(outsideVectors)
    gradOutsideVecs[outsideWordIdx] = (z_o - 1) * centerWordVec
    for k, neg_index in enumerate(negSampleWordIndices):
        gradOutsideVecs[neg_index] += (1 - z_k[k]) * centerWordVec
